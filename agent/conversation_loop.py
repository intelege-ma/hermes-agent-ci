...
# ── LOG-ONLY provider usage tracker singleton ──────────────────────────
# Phase 1b: lazily create one UsageTracker per process and expose it to
# the conversation loop.  Returns None on import/init failure so callers
# can simply skip tracking rather than crash.
...
def _get_usage_tracker():
    """Return the process-wide UsageTracker, or None if unavailable."""
    ...
    with _usage_tracker_lock:
        ...
        try:
            ...
            logger.debug("UsageTracker unavailable — skipping log-only tracking", exc_info=True)
            # Cache None so we don't retry on every API call
            _usage_tracker_instance = None
        return _usage_tracker_instance
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            # ── Advisory pre-flight gate (Phase 2a) ──────────────────
            # LOG-ONLY: checks if the current provider is in cooldown and
            # logs what *would* happen. No provider selection changes.
            # This feeds the soak-test measurement period before Phase 2b
            # enforcement.
            try:
                _ut_adv = _get_usage_tracker()
                if _ut_adv is not None:
                    from agent.provider_router.pre_flight_advisor import PreFlightAdvisor
                    _advisor = PreFlightAdvisor(tracker=_ut_adv)
                    _adv_result = _advisor.check(
                        getattr(agent, "provider", "") or "",
                        model=getattr(agent, "model", "") or None,
                    )
                    if _adv_result.should_skip:
                        logger.info(
                            "ADVISORY: would skip %s/%s — %s "
                            "(advisory=True, enforced=False, remaining=%.0fs)",
                            getattr(agent, "provider", "unknown"),
                            getattr(agent, "model", "unknown"),
                            _adv_result.reason,
                            _adv_result.cooldown_remaining_s,
                        )
            except Exception:
                pass  # Advisory check must never break the agent loop
            ...
            try:
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    # ── LOG-ONLY provider usage tracking (Phase 1b) ──
                    # Record every successful API call to the usage tracker.
                    # Wrapped in try/except so tracker failures never break
                    # the agent loop.  No routing decisions are made here.
                    try:
                        _ut = _get_usage_tracker()
                        if _ut is not None:
                            _ut.record(
                ...
                # ── LOG-ONLY provider error tracking (Phase 1b) ──
                # Record API errors to the usage tracker for later analysis.
                # Wrapped in try/except so tracker failures never break
                # the recovery/retry logic below.
                try:
                    _ut_err = _get_usage_tracker()
                    if _ut_err is not None:
                        _err_provider = getattr(agent, "provider", "") or ""
                        _err_model = getattr(agent, "model", "") or ""
                        if status_code == 429:
                            _retry_after = None
                            _raw_headers = getattr(api_error, "headers", None) or {}
                            _ra_val = _raw_headers.get("retry-after")
                            if _ra_val is not None:
                                try:
                                    _retry_after = float(_ra_val)
                                except (TypeError, ValueError):
                                    pass
                            _ut_err.mark_rate_limited(
                                _err_provider,
                                _err_model,
                                duration_s=_retry_after,
                            )
                        else:
                            _ut_err.record_error_signal(
                                _err_provider,
                                _err_model,
                                status_code=status_code,
                                signal_type="api_error",
                                detail=classified.reason.value,
                            )
                except Exception:
                    pass
